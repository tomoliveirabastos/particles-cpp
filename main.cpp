...
void Particles::reset_particles(std::size_t index, bool start = false)
{
       ...
       sf::Vector2f pos = {
           static_cast<float>(rand()) / (float)RAND_MAX * 10 - 5,
           static_cast<float>(rand()) / (float)RAND_MAX * 10 - 5};
